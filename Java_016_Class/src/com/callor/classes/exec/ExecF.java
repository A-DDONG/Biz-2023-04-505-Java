...
public class ExecF {
	...
	public static void main(String[] args) {
		 ...
		 * 위치 요소로부터     |  데이터를 읽을 수 있다
		 * 값을 읽을수있다     |
		 * println(str[0])     |  특정 index 요소 값을 변경할때는
		 *                     |  List 의 set(index) method 를 통하여 
		 *                     |  값을 변경할 수 있다
		 * 배열은 코드가 실행  |  List는 필요에 따라 size() 가 변경될수 있다
		 * 되는 도중에 크기    |
		 * 변경 불가           |           
		 *                     |  
		 *                     |  remove(index) method 를 통해서
		 *                     |  index 위치의 item 을 제거할수 있다
		 */
		
		// 베열의 크기와 index 를 모르거나 몰라도 될때
		// 전체 배열의 요소를 사용하고 싶을때
