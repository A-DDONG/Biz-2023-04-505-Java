 ...
 * Service 클래스는 주로 비즈니스로직 코드가 작성된다
...
public class ScoreServiceV1 {
	 ...
	 * 이 method 는 클래스 scope 의 method 들만 사용할 수 있도록 하는 것
	 */
	...
	public void scoreListPrint() {
		...
		System.out.println("-".repeat(70));
		for(int i = 0 ; i < scoreList.length ; i++) {
			System.out.printf("%5s\t",scoreList[i].stNum);
...
//		int intArt = 0;
		...
		for(int i = 0 ; i < scoreList.length ; i++) {
			...
			intArt += scoreList[i].scMath;
			
			// 평균의 평균을 출력하기 위한 준비
			// 각 학생의 평균의 총 합산을 계싼하여 floatAvg 에 저장해두기
