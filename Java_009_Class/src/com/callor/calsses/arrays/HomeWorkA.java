...
public class HomeWorkA {
	...
	public static void main(String[] args) {
		// 학생 한명의 점수를 관리하기 위한 객체변수 생성
		// 생성 = 선언 + 초기화 -> 사용할 준비가 된 상태
		// ScoreDto 클래스를 사용하여 score 객체(변수)를 선언
		// ScoreDto() 생성자 method 를 사용하여 객체를 초기화(new) 한다
		ScoreDto score = new ScoreDto();

		// ScoreDto Type scores 배열을 선언하고 요소를 10개 선언한다
		...
		// scores 배열의 개별 요소들을 모두 초기화한다
		// scores 배열의 모든 요소들이 초기화되어 사용할 준비가 완료된다
		for (int i = 0; i < scores.length; i++) {
			scores[i] = new ScoreDto();
		}

		// 학번, 이름, 과목점수를 각 배열의 요소에 세팅
		// ScoreSeriveA 클래스에 정의된
		// 점수생성 method, 개별 성적출력 method 를 활용하기 위해
		// 객체(변수) 로 생성하기
