 ...
 * 		저자(bAuthor), 발행일(bPublishDate), 페이지(bPages), 가격(bPrice) 
 * 
 * OOP 에서 데이터 클래스에 어떤 패턴을 지정할 것인가
 * 		요즘의 Java 에서는 데이터 클래스의 각 속성(요소, Field)의
 * 		접근제한자를 public 으로 선언한다
 * 
 * 		Web Application Service 프로젝트를 구현할때
 * 		특히 SpringMVC 도구를 사용하여 구현할때는
 * 		접근제한자를 private 으로 선언하고
 * 		각 속성에 접근을 하가위해 getter, setter method 를 선언하는 패턴을 사용한다
 */
...
public class BookDto {
	...
	/*
	 * 데이터 클래스 생성할때 변수명 또는 변수의 type 을 변경하고자 할때
	 * getter, setter, toString, 생성자 2개를 모두 지우고
	 * 변수명 또는 변수의 type 을 변경하고
	 * getter, setter, toString, 생성자 2개를 다시 생성하기
	 * 
